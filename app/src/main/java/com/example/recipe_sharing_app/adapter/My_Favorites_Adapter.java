...
public class My_Favorites_Adapter extends RecyclerView.Adapter<My_Favorites_Adapter.RecipeViewHolder> {

    private List<Recipe> recipeList;
    private Context context;

    public My_Favorites_Adapter(List<Recipe> recipeList, Context context) {
    ...
    public void onBindViewHolder(@NonNull RecipeViewHolder holder, int position) {
        ...
        // Handle save icon
        // Handle save icon
        holder.favouriteIcon.setImageResource(R.drawable.favorite); // Set your save icon here

// Add long click listener for 3 seconds to remove from "Save for Later"
        holder.favouriteIcon.setOnLongClickListener(v -> {
            Toast.makeText(holder.itemView.getContext(), "Giữ 3 giây để xóa...", Toast.LENGTH_SHORT).show();
            ...
            new Handler().postDelayed(() -> {
                ...
                new AlertDialog.Builder(holder.itemView.getContext())
                        .setTitle("Xác nhận")
                        .setMessage("Bạn có chắc chắn muốn xóa mục này khỏi 'yêu thích'?")
                        .setPositiveButton("Xóa", (dialog, which) -> {
                            // User confirmed to remove the item
                            removeFromSaveForLater(recipe.getRecipeId(), holder);
                            Toast.makeText(holder.itemView.getContext(), "Đã xóa khỏi 'yêu thich'", Toast.LENGTH_SHORT).show();
                        })
    ...
    private void removeFromSaveForLater(String recipeId, RecipeViewHolder holder) {
        ...
        DatabaseReference favouriteRef = FirebaseDatabase.getInstance().getReference("Favorites");

        // Remove recipe from "Save for Later" in Firebase ( delete)
        favouriteRef.child(recipeId).child(userId).removeValue().addOnSuccessListener(aVoid -> {
            // Notify user that the recipe is removed
            Toast.makeText(holder.itemView.getContext(), "Đã xóa khỏi danh sách love!", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(holder.itemView.getContext(), "Lỗi khi xóa công thức khỏi danh sách love!", Toast.LENGTH_SHORT).show();
        });
    }
    ...
    public static class RecipeViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView authorImageView, recipeImageView, favouriteIcon;
